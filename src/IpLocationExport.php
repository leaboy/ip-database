...
class IpLocationExport {
     ...
     * @param null $filepath
     */
    public final function __construct($filepath = null) {
        $this->init($filepath);
    }
     ...
     * @return array
     */
    private function getAddr($location) {
        $result          = [];
        $is_china        = false;
        $seperator_sheng = '省';
        $seperator_shi   = '市';
...
//            $result['org_area'] = $location['org_area'];
    ...
    public function export($filepath) {
        if (file_exists($filepath)) unlink($filepath); // 删掉重建

        $fp = fopen($filepath, "wa");

        if ($this->fp) {

            $totalip = $this->totalip;
            $nextip = 0;
            ...
            while ($totalip > 0) {
                ...
                if ($nextip == 0) {
                    $nextip = $this->firstip + 7;
                } else {
                    $nextip += 7;
                }

                fseek($this->fp, $nextip);

                $location['beginip'] = long2ip($this->getlong()); // 开始地址
                $offset = $this->getlong3();
                fseek($this->fp, $offset);
                $location['endip'] = long2ip($this->getlong()); // 结束地址
